...
class AsyncBukkitScheduler(private val plugin: Plugin): Scheduler.Worker, Scheduler {
    ...
    override fun schedule(task: Runnable): Disposable {
        ...
        value = Bukkit.getScheduler().runTaskAsynchronously(plugin) {
            task.run()
            tasks.remove(value)
        }
    ...
    override fun schedule(task: Runnable, delay: Long, unit: TimeUnit): Disposable {
        var value: BukkitTask = EmptyTask
        value = Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, {
            task.run()
            tasks.remove(value)
    ...
    override fun schedulePeriodically(task: Runnable, initialDelay: Long, period: Long, unit: TimeUnit): Disposable {
        val value: BukkitTask = Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, {
            task.run()
